...
Ciphertext auxiliaryFtn1p(HomEvaluator eval, Context context, vector<Ciphertext>& ctxtVec, vector<vector<Plaintext>>& ptxtVec, int input_channel) {
    ...
    for (int inputid = 0; inputid < input_channel; ++inputid) {
        ...
        eval.mult(ctxtVec[inputid], ptxtVec[inputid][0], ctxt_out_cache);
...
Ciphertext auxiliaryFtn9p(HomEvaluator eval, Context context, vector<vector<Ciphertext>>& ctxtVec, vector<vector<Plaintext>>& ptxtVec, int input_channel) {
    ...
    for (int inputid = 0; inputid < input_channel; ++inputid) {
        ...
        eval.mult(ctxtVec[inputid][0], ptxtVec[inputid][0], ctxt_out_cache);

        Ciphertext mult_cache(context);
        
        for (int i = 1; i < 9; ++i) {
            eval.mult(ctxtVec[inputid][i], ptxtVec[inputid][i], mult_cache);
...
std::vector<HEaaN::Ciphertext> Convp(HEaaN::Context context, HEaaN::KeyPack pack,
    ...
    if (kernelsize == 9) {
        ...
        for (int outputid = 0; outputid < output_channel; ++outputid) {
            ctxt_out_bundle[outputid] = auxiliaryFtn9p(eval, context, rotated_ctxts_bundle, kernel_o[outputid], input_channel);
        }
        ...
        //#pragma omp parallel for
        for (int outputid = 0; outputid < output_channel; ++outputid) {
            ctxt_out_bundle[outputid] = auxiliaryFtn1p(eval, context, ctxt_bundle, kernel_o[outputid], input_channel);
        }
    }
