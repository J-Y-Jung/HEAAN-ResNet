...
std::vector<std::vector<Ciphertext>> DSB(Context context, KeyPack pack,
    ...
    HEaaNTimer timer(false); // for using timer.
    ///////////////////////// SetUp ////////////////////////////////
    std::cout << "DSB start" << "\n";
    // int num_ctxt;
    ...
    std::vector<std::vector<HEaaN::Ciphertext>> ctxt_conv_out_bundle(16,vector1);//추가
    //#pragma omp parallel
    //#pragma omp for
    /*
    for (int i = 0; i < 16; ++i) { 
        ...
        ctxt_conv_out_bundle[i] = ctxt_conv_out_cache; //추가
    ...
    */
    ...
    std::vector<std::vector<Ciphertext>> ctxt_MPP_in_allch_bundle(4,vector2) //추가
    #pragma omp parallel
    #pragma omp for
    for (int i = 0; i < 4; ++i) {
        ...
        for (int ch = 0; ch < 32; ++ch) {
            ...
            ctxt_MPP_in_allch_bundle[ch] = ctxt_MPP_in_cache;
        }
    ...
    std::vector<HEaaN::Ciphertext> vector3(32,ctxt3); //추가
    std::vector<std::vector<Ciphertext>> ctxt_MPP_out_bundle(4,vector3); //추가
    std::vector<HEaaN::Ciphertext> ctxt_MPP_out(32,ctxt3) //추가
    #pragma omp parallel
    #pragma omp for
    for (int i = 0; i < 4; ++i) {
        std::vector<Ciphertext> ctxt_MPP_out(32,ctxt3);
        #pragma omp parallel
        #pragma omp for
        for (int ch = 0; ch < 32; ++ch) {
            ...
            ctxt_MPP_out[i] = ctxt_MPP_out_cache;
            printf("thread %d : %d (in parallel)\n", omp_get_thread_num(), i);

        }
        //ctxt_MPP_out_bundle.push_back(ctxt_MPP_out);
        ctxt_MPP_out_bundle[i] = ctxt_MPP_out;
    }
    std::cout << "DONE!" << "\n";
    ...
    std::vector<std::vector<Ciphertext>> ctxt_relu_out_bundle(4); //4 추가.
    #pragma omp parallel //추가 
    #pragma omp for //추가
    for (int i = 0; i < 4; ++i) {
        HEaaN::Ciphertext ctxt3(context); //추가
        std::vector<Ciphertext> ctxt_relu_out_allch_bundle(32,ctxt3); //추가
        #pragma omp parallel //추가
        #pragma omp for //추가
        for (int ch = 0; ch < 32; ++ch) {
            std::cout << "(i = " << i << ", " << "ch = " << ch << ")" << "\n";
            ...
            ctxt_relu_out_allch_bundle[ch] = ctxt_relu_out;
        }
        ctxt_relu_out_bundle[i] = ctxt_relu_out_allch_bundle;
    }
    std::cout << "DONE!" << "\n";
