...
4. MPP : DSB에서 MPP 순서를 바꾸면 MPP 횟수는 더 늘어나지만, 연산량 최적화 가능해 보임
...
(추가 예정)원하는 경우 더 컴팩트하게 최대 96개의 이미지의 정보가 한번에 계산되게 변경 가능. (FC64_img96)
  
이의 구현을 위해 원하는 인덱스로 avgpool해주는 AvgpoolToIdx도 필요, 구현 예정.
