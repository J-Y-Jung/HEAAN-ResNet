    ...
    */
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        for (int ch = 0; ch < 16; ++ch) {
            ApproxReLU(context, eval, ctxt_conv1_out_bundle[i][ch], ctxt_relu1_out_bundle[i][ch]);
            if (i==0 && ch ==0){
                dec.decrypt(ctxt_relu1_out_bundle[i][ch], sk, dmsg);
                printMessage(dmsg);
            }
        }
    }
    ...
    dec.decrypt(ctxt_relu1_out_bundle[1][1], sk, dmsg);
    printMessage(dmsg);
