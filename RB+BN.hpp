...
vector<Plaintext>& BN2_add) {
    ...
    for (int i = 0; i < 16/(4*DSB_count); ++i) { // 서로 다른 img
        //std::cout << "i = " << i << std::endl;
        std::vector<HEaaN::Ciphertext> ctxt_conv_out_cache;
        ctxt_conv_out_cache = Conv(context, pack, eval, 32, 1, 1, 16*(2*DSB_count), 16*(2*DSB_count), ctxt_bundle[i], kernel_bundle);
    ...
    */
    ...
    for (int i = 0; i < 16/(4*DSB_count); ++i) {
        std::vector<HEaaN::Ciphertext> ctxt_relu_out_allch_bundle;
        for (int ch = 0; ch < 16*(2*DSB_count); ++ch) {
            std::cout << ch << "\n";
    ...
    for (int i = 0; i < 16/(4*DSB_count); ++i) {
        ...
        ctxt_conv_out2_allch_bundle = Conv(context, pack, eval, 32, 1, 1, 16*(2*DSB_count), 16*(2*DSB_count), ctxt_relu_out_bundle[i], kernel_bundle2);
