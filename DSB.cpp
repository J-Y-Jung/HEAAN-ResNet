    ...
    */
    ...
    for (int o = 0; o < 32; ++o) { // output channel 32
        ...
        for (int i = 0; i < 16; ++i) {   // input channel 16
            ...
            kernel_bundle.clear();
            kernel_bundle.shrink_to_fit();
        }
        kernel_o[o] = kernel_i;
        kernel_i.clear();
        kernel_i.shrink_to_fit();
        //printf("thread %d : %d (in parallel)\n", omp_get_thread_num(), o);
    }
    std::cout << "kernel_msg is " << kernel_msg.getSize() << std::endl;
    //timer.end();
    ...
    for (int o = 0; o < 32; ++o) { // output channel 32
        ...
        for (int i = 0; i < 32; ++i) {   // input channel 32
            ...
            kernel_bundle2.clear();
            kernel_bundle2.shrink_to_fit();
        }
        kernel_o2[o] = kernel_i2;
        kernel_i2.clear();
        kernel_i2.shrink_to_fit();
        //printf("thread %d : %d (in parallel)\n", omp_get_thread_num(), o);
    }
    ...
    for (int o = 0; o < 32; ++o) { // output channel 32
        ...
        for (int i = 0; i < 16; ++i) {   // input channel 16
            ...
            kernel_bundle3.clear();
            kernel_bundle3.shrink_to_fit();
        }
        kernel_o3[o] = kernel_i3;
        kernel_i3.clear();
        kernel_i3.shrink_to_fit();
        //printf("thread %d : %d (in parallel)\n", omp_get_thread_num(), o);
    }
    std::cout << "done" << std::endl;

    //kernel_o.clear();
    //kernel_o.shrink_to_fit();
    int nm = 0;
    while(nm < 1000000){
        ++nm;
    }
    ...
    kernel_o.shrink_to_fit();
    kernel_o2.clear();
    kernel_o2.shrink_to_fit();
    kernel_o3.clear();
    kernel_o3.shrink_to_fit();
