...
int main() {
    ...
    ptxt_zero = ecd.encode(msg_zero, 12, 0);
    
    Ciphertext ctxt_zero(context);
    enc.encrypt(msg_zero, pack, ctxt_zero, 0, 0);

    double cnst = (double)(1.0 / 40.0);
    Ciphertext ctxt_init(context);
    enc.encrypt(msg_zero, pack, ctxt_init, 12, 0);
    ...
    for (int i = (num-1) * 16; i < num*16; ++i) { // 313
        ...
        vector<double> temp;
        txtreader(temp, str);
        imageCompiler(context, pack, enc, 12, temp, imageVec[(i%16)]);
    ...
    for (int i = 0; i < 16; ++i) {
        //#pragma omp parallel num_threads(5)
        {
            ...
            block0conv0summands16[i]=ecd.encode(msg, 11, 0);
        }
    }
    temp0a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[i / 5][i % 5], ctxt_block0relu0_out[i / 5][i % 5]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[8+(i /5)][(i%5)], ctxt_block0relu0_out[8+(i /5)][(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[i / 5][5+(i%5)], ctxt_block0relu0_out[i / 5][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[8+(i /5)][5+(i%5)], ctxt_block0relu0_out[8+(i /5)][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[i / 5][10+(i%5)], ctxt_block0relu0_out[i / 5][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[8+(i /5)][10+(i%5)], ctxt_block0relu0_out[8+(i /5)][10+(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 16; ++i) {
        ReLUnomanage(context, eval, ctxt_block0conv0_out[i][15], ctxt_block0relu0_out[i][15]);
    }
   ...
   // RB 1 - 1
    ...
    {
    kernel_ptxt(context, temp1, block1conv0multiplicands16_16_3_3, 6, 1, 1, 16, 16, 3, ecd);
    }
    temp1.clear();
    temp1.shrink_to_fit();
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        block1conv0summands16[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp1a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 80; ++i) {
        ReLUnomanage(context, eval, ctxt_block1conv0_out[i / 16][i % 16], ctxt_block1relu0_out[i / 16][i % 16]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 80; ++i) {
        ReLUnomanage(context, eval, ctxt_block1conv0_out[5 + (i / 16)][i % 16], ctxt_block1relu0_out[5 + (i / 16)][i % 16]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 80; ++i) {
        ReLUnomanage(context, eval, ctxt_block1conv0_out[10 + (i / 16)][i % 16], ctxt_block1relu0_out[10 + (i / 16)][i % 16]);
    }
    ...
    for (int i = 0; i < 16; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block1conv0_out[15][i % 16], ctxt_block1relu0_out[15][i % 16]);
        }
    }
    ...
    {
    kernel_ptxt(context, temp2, block1conv1multiplicands16_16_3_3, 6, 1, 1, 16, 16, 3, ecd);
    }
    temp2.clear();
    temp2.shrink_to_fit();
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        block1conv1summands16[i] = ecd.encode(msg, 5, 0);
        }
    }
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[i / 5][i % 5], ctxt_block1relu1_out[i / 5][i % 5]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[8+(i /5)][(i%5)], ctxt_block1relu1_out[8+(i /5)][(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[i / 5][5+(i%5)], ctxt_block1relu1_out[i / 5][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[8+(i /5)][5+(i%5)], ctxt_block1relu1_out[8+(i /5)][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[i / 5][10+(i%5)], ctxt_block1relu1_out[i / 5][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[8+(i /5)][10+(i%5)], ctxt_block1relu1_out[8+(i /5)][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 16; ++i) {
        ReLUnomanage(context, eval, ctxt_block1add_out[i][15], ctxt_block1relu1_out[i][15]);
    }
    ...
    {
    kernel_ptxt(context, temp3, block2conv0multiplicands16_16_3_3, 6, 1, 1, 16, 16, 3, ecd);
    }
    temp3.clear();
    temp3.shrink_to_fit();
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        block2conv0summands16[i] = ecd.encode(msg, 5, 0);
        }
    }
    temp3a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[i / 5][i % 5], ctxt_block2relu0_out[i / 5][i % 5]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[8+(i /5)][(i%5)], ctxt_block2relu0_out[8+(i /5)][(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[i / 5][5+(i%5)], ctxt_block2relu0_out[i / 5][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[8+(i /5)][5+(i%5)], ctxt_block2relu0_out[8+(i /5)][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[i / 5][10+(i%5)], ctxt_block2relu0_out[i / 5][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[8+(i /5)][10+(i%5)], ctxt_block2relu0_out[8+(i /5)][10+(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 16; ++i) {
        ReLUnomanage(context, eval, ctxt_block2conv0_out[i][15], ctxt_block2relu0_out[i][15]);
    }
    ...
    {
    kernel_ptxt(context, temp4, block2conv1multiplicands16_16_3_3, 6, 1, 1, 16, 16, 3, ecd);
    }
    temp4.clear();
    temp4.shrink_to_fit();
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        block2conv1summands16[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp4a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[i / 5][i % 5], ctxt_block2relu1_out[i / 5][i % 5]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[8+(i /5)][(i%5)], ctxt_block2relu1_out[8+(i /5)][(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[i / 5][5+(i%5)], ctxt_block2relu1_out[i / 5][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[8+(i /5)][5+(i%5)], ctxt_block2relu1_out[8+(i /5)][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[i / 5][10+(i%5)], ctxt_block2relu1_out[i / 5][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[8+(i /5)][10+(i%5)], ctxt_block2relu1_out[8+(i /5)][10+(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 16; ++i) {
        ReLUnomanage(context, eval, ctxt_block2add_out[i][15], ctxt_block2relu1_out[i][15]);
    }
   ...
   // RB 3 - 1
    ...
    {
    kernel_ptxt(context, temp5, block3conv0multiplicands16_16_3_3, 6, 1, 1, 16, 16, 3, ecd);
    }
    temp5.clear();
    temp5.shrink_to_fit();
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        block3conv0summands16[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp5a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[i / 5][i % 5], ctxt_block3relu0_out[i / 5][i % 5]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[8+(i /5)][(i%5)], ctxt_block3relu0_out[8+(i /5)][(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[i / 5][5+(i%5)], ctxt_block3relu0_out[i / 5][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[8+(i /5)][5+(i%5)], ctxt_block3relu0_out[8+(i /5)][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[i / 5][10+(i%5)], ctxt_block3relu0_out[i / 5][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[8+(i /5)][10+(i%5)], ctxt_block3relu0_out[8+(i /5)][10+(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 16; ++i) {
        ReLUnomanage(context, eval, ctxt_block3conv0_out[i][15], ctxt_block3relu0_out[i][15]);
    }
    ...
    {
    kernel_ptxt(context, temp6, block3conv1multiplicands16_16_3_3, 6, 1, 1, 16, 16, 3, ecd);
    }
    temp6.clear();
    temp6.shrink_to_fit();
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        block3conv1summands16[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp6a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[i / 5][i % 5], ctxt_block3relu1_out[i / 5][i % 5]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[8+(i /5)][(i%5)], ctxt_block3relu1_out[8+(i /5)][(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[i / 5][5+(i%5)], ctxt_block3relu1_out[i / 5][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[8+(i /5)][5+(i%5)], ctxt_block3relu1_out[8+(i /5)][5+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[i / 5][10+(i%5)], ctxt_block3relu1_out[i / 5][10+(i%5)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[8+(i /5)][10+(i%5)], ctxt_block3relu1_out[8+(i /5)][10+(i%5)]);
    }


    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 16; ++i) {
        ReLUnomanage(context, eval, ctxt_block3add_out[i][15], ctxt_block3relu1_out[i][15]);
    }
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block4conv_onebyone_summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp7a.clear();
    ...
    {
    kernel_ptxt(context, temp8, block4conv0multiplicands32_16_3_3, 6, 1, 2, 32, 16, 3, ecd);
    }
    temp8.clear();
    temp8.shrink_to_fit();
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block4conv0summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp8a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block4MPP0_out[i / 10][i % 10], ctxt_block4relu0_out[i / 10][i % 10]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block4MPP0_out[i / 10][10+(i % 10)], ctxt_block4relu0_out[i / 10][10+(i % 10)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block4MPP0_out[i / 10][20+(i % 10)], ctxt_block4relu0_out[i / 10][20+(i % 10)]);
    }
    ...
    for (int i = 0; i < 8; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block4MPP0_out[i / 2][30+(i %2)], ctxt_block4relu0_out[i / 2][30+ (i% 2)]);
        }
    }
    ...
    {
    kernel_ptxt(context, temp9, block4conv1multiplicands32_32_3_3, 6, 2, 1, 32, 32, 3, ecd);
    }
    temp9.clear();
    temp9.shrink_to_fit();
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block4conv1summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp9a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block4add_out[i / 10][i % 10], ctxt_block4relu1_out[i / 10][i % 10]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block4add_out[i / 10][10+(i % 10)], ctxt_block4relu1_out[i / 10][10+(i % 10)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block4add_out[i / 10][20+(i % 10)], ctxt_block4relu1_out[i / 10][20+(i % 10)]);
    }
    ...
    for (int i = 0; i < 8; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block4add_out[i / 2][30+(i %2)], ctxt_block4relu1_out[i / 2][30+ (i% 2)]);
        }
    }
    ...
    {
    kernel_ptxt(context, temp10, block5conv0multiplicands32_32_3_3, 6, 2, 1, 32, 32, 3, ecd);
    }
    temp10.clear();
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block5conv0summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block5conv0_out[i / 10][i % 10], ctxt_block5relu0_out[i / 10][i % 10]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block5conv0_out[i / 10][10+(i % 10)], ctxt_block5relu0_out[i / 10][10+(i % 10)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block5conv0_out[i / 10][20+(i % 10)], ctxt_block5relu0_out[i / 10][20+(i % 10)]);
    }
    ...
    for (int i = 0; i < 8; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block5conv0_out[i / 2][30+(i %2)], ctxt_block5relu0_out[i / 2][30+ (i% 2)]);
        }
    }
    ...
    {
    kernel_ptxt(context, temp11, block5conv1multiplicands32_32_3_3, 6, 2, 1, 32, 32, 3, ecd);
    }
    temp11.clear();
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block5conv1summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp11a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block5add_out[i / 10][i % 10], ctxt_block5relu1_out[i / 10][i % 10]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block5add_out[i / 10][10+(i % 10)], ctxt_block5relu1_out[i / 10][10+(i % 10)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block5add_out[i / 10][20+(i % 10)], ctxt_block5relu1_out[i / 10][20+(i % 10)]);
    }
    ...
    for (int i = 0; i < 8; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block5add_out[i / 2][30+(i %2)], ctxt_block5relu1_out[i / 2][30+ (i% 2)]);
        }
    }
    ...
    {
    kernel_ptxt(context, temp12, block6conv0multiplicands32_32_3_3, 6, 2, 1, 32, 32, 3, ecd);
    }
    temp12.clear();
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block6conv0summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp12a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block6conv0_out[i / 10][i % 10], ctxt_block6relu0_out[i / 10][i % 10]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block6conv0_out[i / 10][10+(i % 10)], ctxt_block6relu0_out[i / 10][10+(i % 10)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block6conv0_out[i / 10][20+(i % 10)], ctxt_block6relu0_out[i / 10][20+(i % 10)]);
    }
    ...
    for (int i = 0; i < 8; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block6conv0_out[i / 2][30+(i %2)], ctxt_block6relu0_out[i / 2][30+ (i% 2)]);
        }
    }
    ...
    {
    kernel_ptxt(context, temp13, block6conv1multiplicands32_32_3_3, 6, 2, 1, 32, 32, 3, ecd);
    }
    temp13.clear();
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        block6conv1summands32[i]=ecd.encode(msg, 5, 0);
        }
    }
    temp13a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block6add_out[i / 10][i % 10], ctxt_block6relu1_out[i / 10][i % 10]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block6add_out[i / 10][10+(i % 10)], ctxt_block6relu1_out[i / 10][10+(i % 10)]);
    }

    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block6add_out[i / 10][20+(i % 10)], ctxt_block6relu1_out[i / 10][20+(i % 10)]);
    }
    ...
    for (int i = 0; i < 8; ++i) {
        #pragma omp parallel num_threads(5)
        {
            ReLUnomanage(context, eval, ctxt_block6add_out[i / 2][30+(i %2)], ctxt_block6relu1_out[i / 2][30+ (i% 2)]);
        }
    }
    ...
    for (int i = 0; i < 40; ++i) {
        ...
        block7conv_onebyone_summands64[i]=ecd.encode(msg, 5, 0);
    }
    #pragma omp parallel for num_threads(40)
    for (int i = 40; i < 64; ++i) {
        Message msg(log_slots, temp14a[i]);
    ...
    {
    kernel_ptxt(context, temp15, block7conv0multiplicands64_32_3_3, 6, 2, 2, 64, 32, 3, ecd);
    }
    temp15.clear();
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        block7conv0summands64[i]=ecd.encode(msg, 5, 0);
    }
    temp15a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block7MPP0_out[0][i], ctxt_block7relu0_out[0][i]);
    }
 ...
 // Second convolution
    ...
    {
        kernel_ptxt(context, temp16, block7conv1multiplicands64_64_3_3, 6, 4, 1, 64, 64, 3, ecd);
    }
    temp16.clear();
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        block7conv1summands64[i]=ecd.encode(msg, 5, 0);
    }
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block7add_out[0][i], ctxt_block7relu1_out[0][i]);
    }
    ...
    {
    kernel_ptxt(context, temp17, block8conv0multiplicands64_64_3_3, 6, 4, 1, 64, 64, 3, ecd);
    }
    temp17.clear();
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        block8conv0summands64[i]=ecd.encode(msg, 5, 0);
    }
    temp17a.clear();
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block8conv0_out[0][i], ctxt_block8relu0_out[0][i]);
    }
    ...
    {
    kernel_ptxt(context, temp18, block8conv1multiplicands64_64_3_3, 6, 4, 1, 64, 64, 3, ecd);
    }
    temp18.clear();
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        block8conv1summands64[i]=ecd.encode(msg, 5, 0);
    }
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block8add_out[0][i], ctxt_block8relu1_out[0][i]);
    }
    ...
    {
    kernel_ptxt(context, temp19, block9conv0multiplicands64_64_3_3, 6, 4, 1, 64, 64, 3, ecd);
    }
    temp19.clear();
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        block9conv0summands64[i]=ecd.encode(msg, 5, 0);
    }
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block9conv0_out[0][i], ctxt_block9relu0_out[0][i]);
    }
    ...
    kernel_ptxt(context, temp20, block9conv1multiplicands64_64_3_3, 6, 4, 1, 64, 64, 3, ecd);
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        block9conv1summands64[i]=ecd.encode(msg, 5, 0);
    }
    ...
    #pragma omp parallel for num_threads(40)
    for (int i = 0; i < 40; ++i) {
        ReLUnomanage(context, eval, ctxt_block9add_out[0][i], ctxt_block9relu1_out[0][i]);
    }
