...
void imageCompiler(Context context, KeyPack pack, Encryptor enc, vector<double>& image, vector<Ciphertext>& output) {
    ...
    for (int i = 0; i < 32; ++i) {
        vector<double> temp1 = slice(image, 3072 * i, 3072 * i + 1023);
        vector<double> temp2 = slice(image, 3072 * i + 1024, 3072 * i + 2047);
        vector<double> temp3 = slice(image, 3072 * i + 2048, 3072 * i + 3071);
